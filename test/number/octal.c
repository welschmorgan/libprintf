...
/*   Updated: 2016/05/08 11:03:55 by mwelsch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		test_basic(t_test *test)
{
	...
	(void)test;
	code = 0;
	code = code >= 0 ? tester("test_oct(0)", "'%o'\n", 0) : code;
	code = code >= 0 ? tester("test_oct(42)", "'%o'\n", 42) : code;
	code = code >= 0 ? tester("test_oct(42, alt)", "'%#o'\n", 42) : code;
	return (code);
}

int		init(int argc, char const *argv[])
{
	init_test_suite(&g_main_test_suite, "number_octal.log", argc, argv);
	push_test_suite(&g_main_test_suite, "number - octal basic", test_basic);
	return (0);
}
